...
export default function OrbitalRingLast({
  ...
  useEffect(() => {
    ...
    const colorB = new THREE.Color("#4285F4"); // Comp 2 Cyan
    ...
    for (let i = 0; i < total; i++) {
      ...
      speeds[i] = 0.1 + Math.random() * 0.1; //particles speed
    ...
    function animate() {
      ...
      // Mouse Following Logic (Comp 2) and speed
      ...
      spritePos.lerp(targetWithDrift, 0.007); // Smooth follow
      ...
      group.rotation.z += 0.0002; // Rotate the ring like a wheel
      group.rotation.x = Math.sin(t * 0.05) * 0.1; // Slight tilt
